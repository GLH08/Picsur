...
export class PreferenceDefaultsService {
  ...
  public getUsrDefault(pref: UsrPreference | string): PrefValueType {
    const defaults = this.usrDefaults as Record<string, (() => PrefValueType) | PrefValueType>;
    const value = defaults[pref as string];
    if (value === undefined) {
      return ''; // Default fallback for undefined preferences
    }
    if (typeof value === 'function') {
      return value();
    } else {
