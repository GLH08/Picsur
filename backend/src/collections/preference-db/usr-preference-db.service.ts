...
export class UsrPreferenceDbService {
  ...
  public async setPreference(
    ...
    // Return
    const valueTypes = UsrPreferenceValueTypes as Record<string, PrefValueTypeStrings>;
    return {
  ...
  public async getPreference(
    ...
    return MutexFallBack(
      ...
      async () => {
        ...
        try {
          existing = await this.usrPreferenceRepository.findOne({
            where: { key: validatedKey, user_id: userid },
            cache: 60000,
          });
      ...
      () => this.saveDefault(userid, validatedKey),
    );
  }
  ...
  public async getAllPreferences(
    ...
    const internalSysPrefs = await Promise.all(
      UsrPreferenceList.map((key: string) => this.getPreference(userid, key)),
    );
    if (internalSysPrefs.some((pref: DecodedUsrPref | Failure) => HasFailed(pref))) {
  ...
  private async saveDefault(
    ...
    const defaultValue = this.defaultsService.getUsrDefault(key as unknown as UsrPreference);
  ...
  private async encodeUsrPref(
    ...
    const validators = UsrPreferenceValidators as Record<string, { safeParse: (v: unknown) => { success: boolean; error?: unknown } } | undefined>;
    const validator = validators[key];
    if (validator) {
      const valueValidated = validator.safeParse(value);
