...
import { FT, HasFailed, ThrowIfFailed } from 'picsur-shared/dist/types/failable';
...
export class JwtStrategy extends PassportStrategy(JwtPassportStrategy, 'jwt') {
  ...
  async validate(payload: any): Promise<EUser | false> {
    ...
    const backendUserRes = await this.usersService.findOne(result.data.uid);
    if (HasFailed(backendUserRes)) {
      if (backendUserRes.getType() === FT.NotFound) return false;
      ThrowIfFailed(backendUserRes);
      // This is unreachable but typescript doesn't know that
      throw new Error('Unreachable');
    }
    const backendUser = backendUserRes;
